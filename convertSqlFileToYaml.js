...
function convertSqlFileToYaml(inputFile, outputFile) {
  try {
    ...
    console.log("Comments removed from the file.");
    ...
    const fleetQueries = queries.map((query, index) => {
      // Generate a simple name for each query (query_1, query_2, etc.)
...
// Input/Output file paths from command-line arguments
...
if (!inputFile || !outputFile) {
  console.error('Usage: node convertSqlFileToYaml.js <input_file.sql> <output_file.yml>');
  process.exit(1);
}
